...
DROP PROCEDURE IF EXISTS searchState$$
...
CREATE PROCEDURE searchState(IN State VARCHAR(100))
    BEGIN
        SELECT nom_est FROM estados WHERE nom_est = State;
    END$$

CREATE PROCEDURE searchColony(IN Colony VARCHAR(100))
    BEGIN
        SELECT nom_mun FROM municipios WHERE nom_mun = Colony;
    END$$
...
CREATE PROCEDURE setCarrito(IN nom VARCHAR(100),precio MEDIUMINT, categoria VARCHAR(30))
    BEGIN
        INSERT INTO Carrito VALUES(categoria, nom, precio);
    END$$
